...
/** Strips lightweight markdown markers to a flat string for compact previews (visit cards). */
fun markdownToPlain(text: String): String = text
    .split("\n")
    .joinToString(" ") { it.trim().removePrefix("## ").removePrefix("# ").removePrefix("- ").removePrefix("• ") }
    .replace(boldRegex, "$1")
    .replace(Regex("\\s+"), " ")
    .trim()
