...
/**
 * Deterministic three-way sync merge (plan phase 1.4). Pure and JVM-unit-testable.
 *
 * Given a common [base] (the snapshot both sides last agreed on), the device's [local] state and the
 * incoming [remote] state, it decides each record's fate WITHOUT relying on a wall clock:
 *
 *  - a record only one side changed  → take the changed side (the other is unchanged vs base);
 *  - a record neither side changed    → unchanged;
 *  - a record present in base but gone on one side, unchanged on the other → honor the delete;
 *  - a delete on one side vs an edit on the other → keep the edit (never lose data to a delete);
 *  - both sides changed the same record differently → KEEP BOTH: a deterministic winner keeps the id,
 *    the loser is preserved as a conflict copy (`<id>~c`) so nothing is silently lost.
 *
 * The winner of a real conflict is chosen by comparing the two encodings lexicographically, so both
 * devices independently pick the same winner and the same copy — the merge converges instead of
 * ping-ponging new copies every round.
 */
object SyncMerge {
    ...
    inline fun <reified T> mergeList(
        ...
        for (id in ids) {
            ...
            when {
                li == null && ri == null -> Unit                          // gone from both
                li == null -> {                                            // deleted locally
                    if (!(bi != null && ri == bi)) out.add(ri!!)          // keep unless remote is the unchanged base
                }
                ri == null -> {                                           // deleted remotely
                    if (!(bi != null && li == bi)) out.add(li)            // keep unless local is the unchanged base
                }
                li == ri -> out.add(li)                                   // identical on both
                bi != null && li == bi -> out.add(ri)                     // only remote changed
                bi != null && ri == bi -> out.add(li)                     // only local changed
                else -> {                                                 // both changed differently → keep both
